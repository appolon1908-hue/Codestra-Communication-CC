...
required=(PGHOST PGPORT PGDATABASE PGUSER PGPASSFILE CODESTRA_COMMUNICATION_BACKUP_ROOT CODESTRA_COMMUNICATION_RECOVERY_WORK_ROOT CODESTRA_RELEASE_SHA CODESTRA_IMAGE_DIGEST CODESTRA_BACKUP_GPG_RECIPIENT CODESTRA_BACKUP_GPG_SIGNING_FINGERPRINT)
...
backup_root=$CODESTRA_COMMUNICATION_BACKUP_ROOT
...
work=$(mktemp -d "$CODESTRA_COMMUNICATION_RECOVERY_WORK_ROOT/${stamp}.XXXXXX")
...
[[ "$database_name" == "codestra_communication" || "$database_name" == "codestra_communication_stage5" ]] || {
...
SCHEMA=codestra-communication-backup.v1
STAMP=$stamp
