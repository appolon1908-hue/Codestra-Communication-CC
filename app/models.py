...
class MessageModel(Base):
    ...
    recipient_ciphertext: Mapped[str | None] = mapped_column(Text, nullable=True)
...
class ConsentModel(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    tenant_id: Mapped[str] = mapped_column(String(128), index=True)
    subject_key: Mapped[str | None] = mapped_column(String(256), nullable=True, index=True)
    subject_ciphertext: Mapped[str | None] = mapped_column(Text, nullable=True)
